...
class SmartContextBuilder:
    ...
    def build_context(self, user_request: str, config: ProjectConfig) -> str:
        ...
        # NEW: Strong prompt constraints to avoid async + databases
        context = f"""
<thinking>
...
Constraints:
- DO NOT use `databases.Database` or any async database abstraction unless the user specifically asks for async.
- DO NOT mix `databases` with SQLAlchemy ORM. Choose one.
- Since the database is {config.database}, use synchronous SQLAlchemy ORM with sessionmaker.
- SQLite lacks reliable async support in production. Use sync code with regular SQLAlchemy.
- Follow best practices: separate Pydantic models, SQLAlchemy models, and use dependency injection for the DB session.
</thinking>

You are a senior Python backend engineer. Generate a complete production-grade Python API.
...
- Use synchronous SQLAlchemy ORM (not databases.Database)
...
"""
    ...
    def validate_generated_code(self, code):
        ...
        try:
            ...
            has_app = 'app = ' in code
            has_framework = 'fastapi' in code.lower() or 'flask' in code.lower()
            ...
            return {
                ...
                "score": sum([has_app, has_framework, has_endpoints]) / 3
            }
        except:
            return {"valid": False, "syntax_ok": False, "score": 0}
...
if __name__ == "__main__":
    main()
    def extract_and_clean_code(self, llm_response):
        """Extract and clean Python code from LLM response with multiple strategies"""
        import re
        
        safe_print("[PROCESSING] Extracting and cleaning generated code...")
        
        # Remove thinking tags and explanations first
        cleaned_response = re.sub(r'<thinking>.*?</thinking>', '', llm_response, flags=re.DOTALL | re.IGNORECASE)
        cleaned_response = re.sub(r'<explanation>.*?</explanation>', '', cleaned_response, flags=re.DOTALL | re.IGNORECASE)
        
        # Strategy 1: Extract from code blocks
        if '```python' in cleaned_response:
            start = cleaned_response.find('```python') + 9
            end = cleaned_response.find('```', start)
            if end != -1:
                code = cleaned_response[start:end].strip()
                if len(code) > 100 and self.validate_generated_code(code)["valid"]:
                    return code
        
        # Strategy 2: Extract from first import
        import re
        import_match = re.search(r'((?:from|import)\s+(?:fastapi|flask).*)', cleaned_response, re.DOTALL | re.IGNORECASE)
        if import_match:
            code = import_match.group(1)
            # Stop at explanations
            stop_match = re.search(r'\n\n(?:Note:|This|The above)', code)
            if stop_match:
                code = code[:stop_match.start()]
            
            if len(code) > 200 and self.validate_generated_code(code)["valid"]:
                return code
        
        # Fallback: Use template
        safe_print("[FALLBACK] Using fallback code due to extraction issues...")
        return self.generate_fallback_code()
    ...
    def generate_fallback_code(self):
        """Generate working fallback code"""
...
@app.get("/")
async def root():
    return {"message": "Welcome to Generated Todo API", "docs": "/docs"}
...
if __name__ == "__main__":
    print("?? Starting Generated Todo API...")
